...
class UpbitWebSocket:
    ...
    async def connect(self) -> bool:
        ...
        """
        try:
            # 🔧 기본 keepalive 사용 (websockets 라이브러리가 자동 관리)
            # ping_interval/ping_timeout 명시 설정 시 asyncio task 파괴 문제 발생
            # recv() timeout으로 블로킹 방지는 이미 처리됨
...
class MyAssetWebSocket:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # 🔧 기본 keepalive 사용 (명시적 ping_interval/ping_timeout은 asyncio task 파괴 문제 유발)
            # 🔧 recv() timeout(60s)으로 블로킹 방지는 이미 처리됨
