...
16. [WebSocket 재연결 및 데이터 구조](#16-websocket-재연결-및-데이터-구조)
17. [포트폴리오 통계](#17-포트폴리오-통계)
18. [자동매수 로직](#18-자동매수-로직-topic-9)
...
#### 구체적 시나리오: 같은 코인의 그룹/설정 변경 추적

**상황**: BTC를 그룹 이동하면서 DCA, 익절, 손절 설정 변경

**1단계: 대형코인 그룹 (설정 A)**
```
그룹: 대형코인
DCA: -5% 간격, 10,000원
익절: +10% (50% 매도) → +15% (50% 매도)
손절: -7% (100% 매도)
group_settings_hash: "aaa111"
```

거래 기록:
```
TR-001 | 대형코인 | BTC | 익절 | +5.2% | +5,200원 | hash: aaa111
TR-002 | 대형코인 | BTC | 손절 | -3.1% | -3,100원 | hash: aaa111
TR-003 | 대형코인 | BTC | 익절 | +4.8% | +4,800원 | hash: aaa111
```
...
**2단계: 중형코인 그룹으로 이동 (설정 B)**
```
그룹: 중형코인
DCA: -10% 간격, 15,000원  ← 변경
익절: +15% (100% 매도)     ← 변경
손절: -10% (100% 매도)     ← 변경
group_settings_hash: "bbb222"
```

거래 기록:
```
TR-004 | 중형코인 | BTC | 익절 | +7.8% | +7,800원 | hash: bbb222
TR-005 | 중형코인 | BTC | 익절 | +8.1% | +8,100원 | hash: bbb222
TR-006 | 중형코인 | BTC | 손절 | -4.2% | -4,200원 | hash: bbb222
```

**통계 (설정 B)**:
- 거래 3건, 익절 2건 (승률 66.7%)
- 평균 수익률: +3.9%  ← **수익률 1.6%p 개선!** ✅

**엑셀 분석 방법**:

1. **코인별 필터**: `symbol = "KRW-BTC"` 필터 → BTC의 모든 거래 조회
2. **그룹별 분류**: `group` 컬럼으로 대형코인/중형코인 분리
3. **설정 추적**: `group_settings_hash`로 같은 설정끼리 그룹핑
4. **통계 비교**:
   - 평균 수익률: 2.3% → 3.9%
   - 익절 평균: 5.0% → 7.95%
   - 손절 평균: -3.1% → -4.2%
5. **결론**: 설정 변경 후 수익률 개선 확인 → 중형코인 설정 유지 결정

**핵심**:
- `group_settings_hash` 덕분에 그룹 이름이 바뀌어도 같은 설정 추적 가능
- 같은 코인(BTC)에 대해 설정 A vs 설정 B의 효과를 **정량적으로 비교** 가능
- 백테스팅 없이 **실거래 데이터로 전략 검증** 가능
...
> **상세 문서**: [`docs/design/TOPIC_9_AUTO_BUY_LOGIC.md`](./design/TOPIC_9_AUTO_BUY_LOGIC.md)

### 18.1 핵심 설계

**Preset 기반 타임프레임 시스템**:
- **보수적 (4시간봉)**: 하루 1~5번, 직장인 적합
- **균형형 (1시간봉)** ⭐ 기본값: 하루 5~15번, 안전+수익 균형
- **적극적 (15분봉)**: 하루 15~30번, 수시 확인 가능자

**기본 지표 (TradingView 공식 설정)**:
- ✅ RSI (14, 30/70) - 과매수/과매도 감지
- ✅ MACD (12, 26, 9) - 추세 전환 포착
- ✅ Volume Surge (2.0배) - 거래량 급증 확인
...
1. **초보자 타겟**: 자동매수의 주 사용자는 초보자 (타점 못 잡음)
2. **검증된 표준**: TradingView 공식 설정 그대로 (조정 불필요)
3. **범용성**: 스캘핑~스윙 모두 커버 가능한 중간 타임프레임
4. **관리 가능성**: 하루 5~15번 정도가 초보자가 매도 타점 관리 가능한 수준
5. **유연한 변경**: 타임프레임/지표 변경 시 전체 구조 영향 <10%

### 18.3 타임프레임별 특성

| 항목 | 4시간봉 | **1시간봉 ⭐** | 15분봉 |
|------|---------|---------------|--------|
| **스캔 빈도** | 300/일 | 1,200/일 | 4,800/일 |
| **예상 거래** | 1~5번/일 | 5~15번/일 | 15~30번/일 |
...
**타임프레임 변경**: WebSocket unit 파라미터만 (1곳) → 영향도 5%
**지표 파라미터**: GUI 기본값만 (1곳) → 영향도 3%
**지표 추가/제거**: Strategy 클래스만 (1파일) → 영향도 10%
**Preset 변경**: 사용자 선택만 → 영향도 1%

**결론**: 나중에 수정해도 V4 전체 구조 영향 <10% ✅

### 18.6 백테스트 계획

**데이터**: BTC/ETH/XRP 1년치 1시간봉
**전략**: RSI(14) + MACD(12,26,9) + Volume(2.0x)
**목표**: 실제 신호 빈도 확인 (현재 추정치)
**소요 시간**: 약 1시간

> **상세 내용 (웹서치 근거, 타임프레임 비교, 지표 분석 등)은 별도 문서 참조**:
> [`docs/design/TOPIC_9_AUTO_BUY_LOGIC.md`](./design/TOPIC_9_AUTO_BUY_LOGIC.md)
...
- 2025-01-24 (v10): **자동매수 로직 설계 - Preset 타임프레임, TradingView 표준 지표, 초보자 타겟 (Topic 9 완료)**
