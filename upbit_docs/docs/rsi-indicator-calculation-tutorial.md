...
- **누적 평균 상승폭, 누적 평균 하락폭 계산 공식**
  ...
  - 누적 평균 하락폭 = 누적 평균 하락폭 = (누적 평균 하락폭 \* (기간 - 1) + 전일 종가 대비 변화 금액(음수)) / 기간  
    <br />
...
디지털 자산의 전일 종가는 [일(Day) 캔들 조회](ref:list-candles-days) API를 호출하여 쉽게 확인할 수 있습니다. 이 가이드에서는 비트코인의 캔들 데이터를 활용해 14일 동안의 평균 상승폭과 평균 하락폭을 구하고 RS와 RSI를 계산합니다.
