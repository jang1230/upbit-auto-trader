...
API 요청 처리 중 오류가 발생한 경우 HTTP 응답 본문(Body)에 에러 코드가 함께 반환됩니다. 주요 에러 코드는 [REST API 사용 안내](ref:rest-api-guide) 페이지 및 각 API Reference 문서 우측 하단 응답 예시에서 확인하실 수 있습니다.  
...
파라미터에 : 또는 +와 같은 특수 문자가 포함되었을 때(주로 날짜 형식), 쿼리 문자열을 URL 인코딩 없이 요청 하는 경우 위와 같은 에러가 발생할 수 있습니다. [REST API 사용 및 에러 안내](ref:rest-api-guide) 페이지 또는 [첫 업비트 API 호출하기](doc:first-quotation-api-call) 페이지를 참고하여 URL 인코딩 후 요청해주세요.
