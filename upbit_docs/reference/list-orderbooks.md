...
종목별 호가 단위에 따라 지원하는 모아보기 단위가 다릅니다. 지원하는 모아보기 단위 정보는 [마켓별 주문 정책](doc:faq-market-policy) 문서 또는 [호가 정책 조회](ref:list-orderbook-instruments) API 응답을 참고하여 사용해주시기 바랍니다. 미지원 단위를 지정하여 요청하는 경우 빈 배열이 반환되므로 호출 전 지원하는 단위를 반드시 확인해주시기 바랍니다.
