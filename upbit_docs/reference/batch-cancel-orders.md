...
예약 주문(WATCH) 상태의 주문은 일괄 주문 취소로 취소할 수 없으므로 취소를 원하시는 경우 [개별 주문 취소 접수](ref:cancel-order) 또는 [id로 주문 목록 취소 접수](ref:cancel-orders-by-ids) API를 사용해주세요.
...
취소 대상 페어 또는 대상 마켓에 포함된 페어 중 취소 제외 페어(excluded_pairs)에 포함된 페어의 경우, **취소 제외 페어 지정을 높은 우선순위로 적용**하여 취소 대상에서 제외됩니다.
